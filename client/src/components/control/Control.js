...
export const Control = () => {
  ...
  function makeRooms(width, height, padding, room_count) {
    const rooms = [];
  ...
  function randomize(width, height, padding) {
    let rooms = makeRooms(width, height, padding, 1000);

    // Initializng tile states
    const tile_states = [...Array(height)].map((e) =>
      Array(width)
        .fill()
        .map((u) => ({ room: 0, on: false, color: "black" }))
    );
    ...
    for (let i = 0; i < rooms.length; i++) {
      ...
      for (let x = x1; x < x2 + 1; x++) {
        for (let y = y1; y < y2 + 1; y++) {
          tile_states[y][x] = { room: 0, on: true, color: "white" };
        }
      }
    }
    ...
    for (let i = 0; i < width; i++) {
      for (let j = 0; j < height; j++) {
        if (tile_states[j][i]["on"] && tile_states[j][i]["room"] === 0) {
          room_index += 1;
          let stack = [];
          stack.push([i, j]);
    ...
    dispatch(setTilesCondition(tile_states));
  }
